import { bool, bs, compile, data, int, list, pInt, pfn, plet, pmatch, pstruct } from "@harmoniclabs/plu-ts";
...
export const endpointValidator = pfn([
    ...
    data
], bool)((datum, redeemer, ctx) => {
    return pmatch(redeemer)
        .onSend(({ message }) => {
        const validSender = plet(datum.config.owner.eq(message.sender));
        const validNonce = plet(message.nonce.gt(pInt(0)));
        const validMessage = plet(message.message.length.gt(pInt(0)).and(message.message.length.lt(pInt(1024))));
        const validEndpoint = plet(message.srcEid.eq(datum.config.endpointId));
        return validSender.and(validNonce).and(validMessage).and(validEndpoint);
    })
        .onReceive(({ message, proof }) => {
        const validProof = plet(proof.length.gt(pInt(0)));
        const validInboundNonce = plet(message.nonce.gt(pInt(0)));
        const validDestination = plet(message.receiver.length.gt(pInt(0)));
        return validProof.and(validInboundNonce).and(validDestination);
    })
        .onSetConfig(({ newConfig }) => {
        const isOwner = plet(datum.config.owner.eq(newConfig.owner));
        const validEndpointId = plet(datum.config.endpointId.eq(newConfig.endpointId));
        return isOwner.and(validEndpointId);
    })
        .onSetDelegate(({ delegate, approved }) => {
        const isOwner = plet(datum.config.owner.length.gt(pInt(0)));
        const validDelegate = plet(delegate.length.gt(pInt(0)));
