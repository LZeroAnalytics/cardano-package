...
devnet_generator = import_module("./src/cardano_node/devnet_generator.star")
...
def run(plan, args={}):
    ...
    plan.print("Generating local devnet config and keys...")
    devnet = devnet_generator.generate_local_devnet(plan, config.cardano_params, faucet.address, config.prefunded_accounts)

    plan.print("Deploying Cardano node...")
    cardano_context = cardano_node.launch_cardano_node(
        plan,
        devnet.files_artifact_name,
        config.cardano_params.network_magic
    )
